...
import { Reveal, Stagger, revealVariants, easePremium } from '../components/motion/Reveal'

const SORT_OPTIONS = [
  { value: '-created_at', label: 'Newest' },
  { value: 'created_at',  label: 'Oldest' },
  { value: '-sgv_score',  label: 'Top Rated' },
  { value: 'sgv_score',   label: 'Lowest Score' },
]
...
export default function Explore() {
  ...
  const params = useMemo(() => ({
    ...
    ordering: sort,
    page,
  }), [debounced, filters.category, filters.status, filters.min_score, filters.max_score, filters.date_from, filters.date_to, sort, page])
        ...
        <div className={`flex gap-6 ${showFilters ? 'lg:grid lg:grid-cols-4' : ''}`}>
          ...
          <AnimatePresence>
            {showFilters && (
              ...
              >
                <IdeaFilters filters={filters} onChange={setParam} />
              </motion.div>
            )}
          </AnimatePresence>
