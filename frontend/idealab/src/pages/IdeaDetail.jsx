...
export default function IdeaDetail() {
  ...
  const statusMutation = useMutation({
    mutationFn: (s) => changeIdeaStatus(id, s),
    onSuccess: () => {
  ...
  const canVerdict = user?.role === 'reviewer' && ['submitted', 'review'].includes(idea.status)
    ...
    <div className='min-h-screen bg-primary'>
      ...
      >
        ...
        <div className='mt-6 flex justify-end gap-3'>
          <Button variant='ghost' onClick={() => { setVerdictOpen(false); setPendingStatus(null) }}>Cancel</Button>
          <Button
