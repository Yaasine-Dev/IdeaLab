...
export default function ReviewerDashboard() {
  ...
  const distribution = useMemo(() => [
    { label: '0–2.5',  count: scores.filter((s) => s < 2.5).length },
    { label: '2.5–5',  count: scores.filter((s) => s >= 2.5 && s < 5).length },
    { label: '5–7.5',  count: scores.filter((s) => s >= 5 && s < 7.5).length },
    { label: '7.5–10', count: scores.filter((s) => s >= 7.5).length },
  ], [scores])
        ...
        <div className='grid gap-6 lg:grid-cols-2'>
          ...
          <div className='rounded-2xl border border-secondary/12 bg-primary p-6 shadow-[0_4px_24px_rgba(104,26,21,0.06)]'>
            ...
            <div className='flex items-center justify-between text-xs text-secondary/35'>
              ...
              {levelCfg.next
                ? <span>{levelCfg.max - rep} pts to {levelCfg.next}</span>
                      ...
                      >
                        ...
                        <td className='px-6 py-4'>
                          <span className='font-display text-lg font-black text-secondary'>
                            {row.my_score}<span className='text-xs text-secondary/30'>/10</span>
                          </span>
                        </td>
